...
export class RegistrerMemberComponent implements OnInit {
  ...
  ngOnInit() {
    this.form = this._fb.group({
      ci: [this.data.edit ? this.data.ci : '', [Validators.required, Validators.pattern(/^[V|E|J|P][0-9]{5,9}$/,)]],
      ...
      phone: [this.data.edit ? this.data.phone : '', [Validators.required, Validators.pattern(/^(0212|0414|0424|0412|0416|0426)[-][0-9]{7}$/g)]],
      gender: [this.data.edit ? this.data.gender : '', [Validators.required]],
      dateOfBirth: [this.data.edit ? new Date(this.data.dateOfBirth).toISOString().substr(0, 10) : '', [Validators.required]],
      apartment: [this.data.edit ? this.data.apartment : '', [Validators.required]],
